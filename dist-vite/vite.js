...
export function maplibreWorker(options = {}) {
    let outDir = 'dist';
        ...
        async closeBundle() {
            ...
            for (const name of options.files ?? WORKER_FILES) {
                try {
                    ...
                    throw new Error(`urban-climate-map: could not copy ${name} from ${dist}. ` +
                        'Without it a production build renders no vector data.', { cause });
                }
            }
        },
    };
